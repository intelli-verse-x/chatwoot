...
Model Context Protocol (MCP) server for **chatwoot**, part of the Intelli-Verse engagement stack.

- Transport: MCP JSON-RPC 2.0 over HTTP (`POST /`), liveness at `GET /healthz`.
- Auth: **per-request** — the caller passes the chatwoot API token via `Authorization: Bearer <token>`.
...
`intelli-verse-kube-infra/chatwoot-mcp/deployment.yaml` (namespace `aicart`).
